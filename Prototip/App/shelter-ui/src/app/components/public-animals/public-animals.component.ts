...
@Component({
  selector: 'app-public-animals',
  imports: [FormsModule, RouterLink],
  templateUrl: './public-animals.component.html',
  styleUrl: './public-animals.component.css',
})
export class PublicAnimalsComponent implements OnInit {
  ...
  protected get filteredAnimals(): Animal[] {
    const q = this.query.trim().toLowerCase();
    return !q
      ? this.animals
      : this.animals.filter(
          (a) => a.name.toLowerCase().includes(q) || a.species.toLowerCase().includes(q),
        );
  }
